...
import arcpy, logging, os, sys, datetime, re
...
def fieldTypeConverter(output_temp):
    ...
    for field in fields:
        ...
        # need an alias defined because of schema issues with the spreadsheet
        ...
        if field_name == 'ID':
            if not field_type == 'Long':
                logger.info('')
                logger.info('field name is : {0} and field type is: {1}'.format(field_name, field_type))
                logger.info('transferring values to new fields...')
                ### using this tool to transfer the vales to a long format
                logger.info('converting field to the long format...')
                arcpy.AddField_management(in_table=output_temp,
                                                  field_name='temp',
                                                  field_type='Long',
                                                  field_precision='',
...
def calcLatLong(output):
    logger.info('if location value has data errors, co-ord -26.006099,133.952746 is assigned...')
    # define variables
